...
/**
 * Query for {@code DELETE /subscriptions} — removes a webhook subscription.
 *
 * <p>Example usage:</p>
 * <pre>{@code
 * api.unsubscribe("https://example.com/webhook").execute();
 * }</pre>
 */
public class UnsubscribeQuery extends MaxQuery<SimpleQueryResult> {

    /**
     * Creates an UnsubscribeQuery.
     *
    ...
    public UnsubscribeQuery(MaxClient client, String url) {
        super(client, "/subscriptions", HttpMethod.DELETE, SimpleQueryResult.class);
        queryParams.put("url", Objects.requireNonNull(url, "url must not be null"));
    }
}
